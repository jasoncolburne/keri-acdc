...
pub fn expand_acdc(
    ...
    keri::saidify_value(&mut crd, Some(Ids::d), Some(false), Some(true))?;
    Creder::new_with_ked(&crd, Some(&creder.code()), Some(&creder.kind()))
}
...
pub fn issue_acdc(
    ...
    let (ixn_said, ixn) = keri::kmi::interact(&keys, issuer, &dig, sn, &data)?;
    drop(keys);
...
pub fn revoke_acdc(
    ...
    let (ixn_said, ixn) = keri::kmi::interact(&key_set, issuer, &dig, sn, &data)?;
    drop(key_set);
