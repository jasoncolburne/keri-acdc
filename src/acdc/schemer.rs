...
pub(crate) fn schema_cache() -> &'static mut SchemaCache {
...
mod test {
    ...
    fn schemas() {
        ...
        assert!(cache()
            .verify(
                &schemers[0].said().unwrap(),
                "{\"d\":\"foo\",\"i\":\"bar\"}"
            )
            .is_ok());
        assert!(cache()
            .verify(&schemers[0].said().unwrap(), "{\"i\":\"bar\"}")
            .is_err());
        assert!(cache()
            .verify(&schemers[0].said().unwrap(), "{\"d\":\"foo\"}")
            .is_err());
        assert!(cache()
            .verify(
                &schemers[0].said().unwrap(),
